...
export function useMandateData({ documentSymbol }: UseMandateDataParams): MandateDataResponse {
  ...
  useEffect(() => {
    const fetchMandateData = async () => {
      ...
      try {
        ...
        const encodedSymbol = documentSymbol.split('/').map(segment => encodeURIComponent(segment)).join('/')
