...
function getHighlights(a: string | null, b: string | null): [string | null, string | null] {
  a = a || "";
  b = b || "";
  ...
  try {
    ...
    if (symbolA && symbolB) {
      ...
      return NextResponse.json(
        { error: 'Either document symbols (symbolA, symbolB) or file paths (aPath, bPath) are required' }, 
        { status: 400 }
      );
    }
    ...
    while (true) {
      ...
      for (let _i = i + 1; _i < a.length; _i++) {
        ...
        if (ratio(_aa, bb) > THRESHOLD) {
          ...
          for (let __i = i + 1; __i < _i; __i++) {
            const __aa = a[__i];
            const bestMatch = b.reduce((best: { line: string | null; ratio: number }, _bb: string) => {
            ...
            const [leftHighlighted, rightHighlighted] = getHighlights(__aa, bestMatchLine);
      ...
      if (!aa) {
        const bestMatchResult = a.reduce((best: { line: string | null; ratio: number }, _aa: string) => {
          const currentRatio = ratio(_aa, bb);
          return currentRatio > best.ratio ? { line: _aa, ratio: currentRatio } : best;
        }, { line: null, ratio: 0 });
        
        bestMatch = bestMatchResult.ratio > THRESHOLD ? bestMatchResult.line : null;
      }
      
      const [leftHighlighted, rightHighlighted] = getHighlights(aa || bestMatch, bb);
    ...
    const scores = ab.filter(item => item.score !== null).map(item => item.score as number);
    ...
    return NextResponse.json({ error: 'Failed to process diff' }, { status: 500 });
  }
}
