  ...
  try {
    ...
    recordArray.forEach((record: XMLRecord) => {
      ...
      if (record.datafield) {
        ...
        datafields.forEach((field: XMLField) => {
          ...
          if (!title && field['@_tag'] === '245' && field.subfield) {
            ...
            ['a', 'b', 'c'].forEach(code => {
              const subfield = subfields.find((sf: XMLSubfield) => sf['@_code'] === code);
          ...
          if (!title && field['@_tag'] === '239' && field['@_ind1'] === '1' && field['@_ind2'] === '0') {
            if (field.subfield) {
              const subfields = Array.isArray(field.subfield) ? field.subfield : [field.subfield];
              const titleSubfield = subfields.find((sf: XMLSubfield) => sf['@_code'] === 'a');
          ...
          if (field['@_tag'] === '191' && field.subfield) {
            const subfields = Array.isArray(field.subfield) ? field.subfield : [field.subfield];
            const symbolSubfield = subfields.find((sf: XMLSubfield) => sf['@_code'] === 'a');
          ...
          if (field['@_tag'] === '260' && field.subfield) {
            const subfields = Array.isArray(field.subfield) ? field.subfield : [field.subfield];
            const dateSubfield = subfields.find((sf: XMLSubfield) => sf['@_code'] === 'c');
          ...
          if (field['@_tag'] === '269' && field.subfield) {
            const subfields = Array.isArray(field.subfield) ? field.subfield : [field.subfield];
            const isoDateSubfield = subfields.find((sf: XMLSubfield) => sf['@_code'] === 'a');
      ...
      if (record.controlfield && !symbol) {
        const controlfields = Array.isArray(record.controlfield) ? record.controlfield : [record.controlfield];
        controlfields.forEach((cf: XMLControlField) => {
          if (cf['@_tag'] === '001' && cf['#text']) {
            ...
            const symbolMatch = cfText.match(/([A-Z]+\/[A-Z]*\/?[\d]+(?:\s*\([^)]+\))?)/);
            if (symbolMatch) {
      ...
      if (title && !symbol) {
        const symbolMatch = title.match(/([A-Z]+\/[A-Z]*\/?[\d]+(?:\s*\([^)]+\))?)/);
        if (symbolMatch) {
          symbol = symbolMatch[1];
        }
      }
    ...
    return NextResponse.json(
      { error: 'Failed to fetch resolutions', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
