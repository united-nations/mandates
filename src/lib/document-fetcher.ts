...
export async function fetchDocument(symbol: string): Promise<DocumentData> {
  ...
  if (!response.ok) {
    ...
    if (!fallbackResponse.ok) {
      throw new Error(`Failed to fetch document ${symbol}: ${response.status} ${response.statusText}`);
    }
...
async function extractDocument(response: Response, symbol: string): Promise<DocumentData> {
  const buffer = await response.arrayBuffer();
  const tempFilePath = join(tmpdir(), `${symbol.replace(/\//g, '_')}_${Date.now()}.docx`);
