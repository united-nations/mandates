...
export function HierarchicalSelect({
  ...
  const grouped = React.useMemo(() => {
    const map = new Map<string, { items: HierarchicalOption[], isFlat: boolean }>()
    ...
    map.forEach((value, key) => {
      ...
      const shouldFlatten = items.length === 1 || items.every(item => item.level1 === item.level2)
  ...
  const level1Order = ['General Assembly', 'Security Council', 'ECOSOC', 'Treaty Bodies', 'Secretariat', 'Other']
  const level2Order: Record<string, string[]> = {
    'General Assembly': ['Plenary', 'Resolutions', 'Human Rights Council', 'Subsidiary Bodies'],
    'Security Council': ['Plenary', 'Resolutions', 'Presidential Statements', 'Subsidiary Bodies'],
    'ECOSOC': ['Plenary', 'Executive Boards', 'Functional Commissions', 'Committees', 'Regional Commissions', 'Conferences', 'Expert Groups'],
  }

  // Sort function for level1
  const sortLevel1 = (groupedData: Map<string, { items: HierarchicalOption[], isFlat: boolean }>) => (a: string, b: string) => {
    if (sortByCount) {
      // Sort by total count (descending)
      const countA = groupedData.get(a)?.items.reduce((sum, item) => sum + (item.count || 0), 0) || 0
      const countB = groupedData.get(b)?.items.reduce((sum, item) => sum + (item.count || 0), 0) || 0
  ...
  const sortLevel2 = (level1: string) => (a: HierarchicalOption, b: HierarchicalOption) => {
    if (sortByCount) {
    ...
    if (indexA === -1 && indexB === -1) return a.level2.localeCompare(b.level2)
    if (indexA === -1) return 1
    if (indexB === -1) return -1
    return indexA - indexB
  }
  ...
  const filtered = React.useMemo(() => {
    ...
    const result = new Map<string, { items: HierarchicalOption[], isFlat: boolean }>()
          ...
          {filtered.size === 0 ? (
            ...
            <div className="space-y-1">
              {Array.from(filtered.entries())
                ...
                const allKeys = items.map(item => `${item.level1}|${item.level2}`)
                ...
                if (isFlat) {
                  ...
                  return (
                    <div key={level1} className="flex w-full items-center gap-2">
                      ...
                      >
                        ...
                        <span className="text-xs text-gray-500 shrink-0">{totalCount}</span>
                      </button>
                    </div>
                  )
                }
                      ...
                      >
                        <div
                          className={cn(
                            ...
                            someSelected ? "bg-blue-600 border-blue-600" : "border-gray-300"
                          )}
                        >
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="flex-1 text-left text-gray-700 min-w-0">
                                {showPrefix && item.prefix && <span className="font-mono text-xs text-blue-600 mr-2">{item.prefix}</span>}
                                {fullName ? (
                                  <>
