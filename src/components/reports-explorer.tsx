...
export function ReportsExplorer() {
  ...
  const [clusterMetadata, setClusterMetadata] =
    useState<ClusterMetadata | null>(null);
  ...
  const [expandedSimilar, setExpandedSimilar] = useState<Set<string>>(
    new Set(),
  );
  ...
  const organOptions = useMemo((): HierarchicalOption[] => {
    const organMap = new Map<
      string,
      Map<string, { prefix: string; count: number }>
    >();
    ...
    const reportsForCounts = reports.filter((report) => {
      ...
      if (
        report.year &&
        (report.year < yearRange[0] || report.year > yearRange[1])
      )
        return false;
      return true;
    });
  ...
  const stats = useMemo(() => {
    ...
    const totalClustered = filteredReports.filter(
      (r) => r.cluster_id !== null,
    ).length;
    const totalSeries = new Set(
      filteredReports
        .filter((r) => r.is_recurring_series)
        .map((r) => r.normalized_title),
    ).size;
    const avgSimilarity =
      clusteredReports.clusters.size > 0
  ...
  const getSimilarReports = (report: Report) => {
    if (!report.top10_similar_symbols || !report.top10_similar_scores)
      return [];
    ...
    if (viewMode === "list") {
      ...
      const sortedClusters = clusterArray.sort((a, b) => {
        if (clusterSort === "size") {
          ...
          const avgSimA =
            a[1].reduce((sum, r) => sum + (r.cluster_similarity || 0), 0) /
            a[1].length;
          const avgSimB =
            b[1].reduce((sum, r) => sum + (r.cluster_similarity || 0), 0) /
            b[1].length;
  ...
  const exportClusterAnalysis = () => {
    const analysis = {
      ...
      clusters: Array.from(clusteredReports.clusters.entries()).map(
        ([clusterId, reports]) => ({
          id: clusterId,
          title:
            clusterMetadata?.cluster_details?.[clusterId.toString()]?.title ||
            `Cluster ${clusterId}`,
          description:
            clusterMetadata?.cluster_details?.[clusterId.toString()]
              ?.description,
          suggestion:
            clusterMetadata?.cluster_details?.[clusterId.toString()]
              ?.suggestion,
            ...
            {pageNumbers.map((pageNum, idx) =>
              pageNum === -1 ? (
                ...
                <Button
                  key={pageNum}
                  variant={
                    currentPage === pageNum ? "default" : "outline-solid"
                  }
                  size="sm"
  ...
  const ReportCard = ({
    ...
    const seriesReports = isNested
      ? []
      : getSeriesReports(report.normalized_title || "", report.organ || "");
    ...
    return (
      <div
        className={`border rounded-lg p-4 ${isNested ? "bg-gray-50 border-gray-200" : "bg-white"} mb-2`}
      >
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1">
            <div className="mb-3">
              ...
              {report.statement_of_responsibility &&
                report.statement_of_responsibility.length > 0 && (
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {!isNested && report.cluster_id !== null && (
                <Badge variant="outline" className="text-xs">
                  {clusterMetadata?.cluster_titles?.[
                    report.cluster_id.toString()
            ...
            {seriesReports.length > 0 && (
              <div>
                ...
                >
                  ...
                  {seriesReports.length} previous version
                  {seriesReports.length !== 1 ? "s" : ""}
                </button>
                {isSeriesExpanded && (
    ...
    <div className="space-y-6">
      ...
      <div className="bg-linear-to-r from-blue-50 to-indigo-50 p-4 sm:p-6 rounded-lg border border-blue-200">
        ...
        <p className="text-sm sm:text-base text-gray-700 leading-relaxed">
          Analysis of UN reports published by the UN Digital Library from
          2020–2025.
        </p>
        <p className="text-sm sm:text-base text-gray-700 leading-relaxed">
          Clusters are based on semantic similarity, with automatically
      ...
      <div className="bg-white p-6 rounded-lg border space-y-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            <Select
              value={clusterSort}
              onValueChange={(v: "size" | "similarity") => setClusterSort(v)}
            >
        ...
        {hasActiveFilters && (
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {selectedDocumentTypes.size > 0 && (
              <>
                {Array.from(selectedDocumentTypes)
                  .slice(0, 3)
                  .map((key) => {
                    const [level1, level2] = key.split("|");
                ...
                {selectedDocumentTypes.size > 3 && (
                  <Badge variant="secondary">
                    +{selectedDocumentTypes.size - 3} more
                  </Badge>
                )}
              </>
            )}
            ...
            {(yearRange[0] !== 2020 || yearRange[1] !== 2025) && (
              <Badge variant="secondary" className="gap-2">
                ...
                <button
                  onClick={() => setYearRange([2020, 2025])}
                  className="ml-1 hover:text-red-600"
                >
                  ×
                </button>
        ...
        <div className="flex justify-between items-center text-sm text-gray-600">
          <span>
            ...
            {viewMode === "cluster" &&
              ` (${clusteredReports.clusters.size} clusters)`}
          </span>
        </div>
      </div>
      ...
      <div>
        {viewMode === "list" ? (
          ...
          <div className="space-y-4">
            {/* Clustered reports */}
            {(paginatedData.items as [number, Report[]][]).map(
              ([clusterId, reports]) => (
                <ClusterCard
                  key={clusterId}
                  clusterId={clusterId}
      ...
      <div className="bg-linear-to-br from-blue-50 to-purple-50 rounded-lg border-2 border-blue-200 p-6 mt-8">
        ...
        <p className="text-sm text-gray-600 mb-4">
          From the UN80 report "Shifting Paradigms" (paragraph 38), presented to
          maximize the value of meetings and reports:
        </p>

        <div className="space-y-3">
          <div className="p-4 bg-white rounded-lg border border-gray-300">
            <p className="text-sm">
              <span className="font-semibold text-gray-900">1.</span> Prioritize
              and streamline requests in mandates for reports or meetings
            </p>
          </div>

          <div className="p-4 bg-white rounded-lg border border-gray-300">
            <p className="text-sm">
              <span className="font-semibold text-gray-900">2.</span> Provide
              shorter reports and reduce maximum word counts
            </p>
          </div>

          <div className="p-4 bg-white rounded-lg border border-gray-300">
            <p className="text-sm">
              <span className="font-semibold text-gray-900">3.</span> Combine
              reports covering similar issues and contexts wherever feasible
              (for example by merging separate regional reports on the same
              issue or submitting one report to different mandating bodies
              requesting reports on similar issues)
            </p>
          </div>

          <div className="p-4 bg-white rounded-lg border border-gray-300">
            <p className="text-sm">
              <span className="font-semibold text-gray-900">4.</span> Introduce
              different report formats based on needs and content type; first
              reports could be longer, followed by shorter updates, visual
              dashboards, in-person briefings or other formats
            </p>
          </div>

          <div className="p-4 bg-white rounded-lg border border-gray-300">
            <p className="text-sm">
              <span className="font-semibold text-gray-900">5.</span> Publish
              download statistics for all reports to inform further discussions
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <p className="text-sm text-gray-800">
            <strong>About this analysis:</strong> Use the "Export Analysis"
            button above to download detailed cluster information. The
            clustering analysis uses machine learning to identify similar
            reports based on content.
          </p>
        </div>
      </div>
    </div>
